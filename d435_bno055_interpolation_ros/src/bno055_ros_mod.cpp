...
bno055_arduino_mod::bno055_arduino_mod(ros::NodeHandle &n_imu)
{
    device_ = "/dev/ttyACM0";
...
int bno055_arduino_mod::openPort()
{
    try
    {
...
void bno055_arduino_mod::update()
{
    std::string buffer;
...
void bno055_arduino_mod::publish_data()
{
    imu_publisher.publish(imu_msg);
    mag_publisher.publish(mag_msg);
}

void bno055_arduino_mod::closePort()
{
    Serial_port.Close();
}
