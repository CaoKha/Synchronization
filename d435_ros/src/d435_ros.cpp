...
realsenseD435::realsenseD435(ros::NodeHandle &n_image)
{
    width_ = 640;
    height_ = 480;
    fps_ = 30;
...
cv::Mat &realsenseD435::fix_depth_scale(const cv::Mat &from_image, cv::Mat &to_image)
{
...
void realsenseD435::publishFrame(rs2::frame f, const ros::Time &t, const rs2_stream &stream,
...
void realsenseD435::publishDepthandColor(rs2::frameset frames, ros::Time &t)
{
...
void realsenseD435::publishAlignedDepth(rs2::frameset frames, const ros::Time &t)
{
    for (auto it = frames.begin(); it != frames.end(); ++it)
    {
        auto frame = (*it);
