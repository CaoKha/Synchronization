...
mpu9255_arduino::mpu9255_arduino(ros::NodeHandle &n_imu)
{
    device_ = "/dev/ttyACM0";
    imu_publisher = n_imu.advertise<sensor_msgs::Imu>("imu/imu_raw", 2);
    mag_publisher = n_imu.advertise<sensor_msgs::MagneticField>("imu/mag", 2);
    imu_sequence = 0;
}

int mpu9255_arduino::openPort()
{
    try
    {
...
void mpu9255_arduino::closePort()
{
    Serial_port.Close();
}